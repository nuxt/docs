...
这就是Nuxt提供更高阶**模块系统**的原因，可以轻松扩展核心。 模块只是在引导Nuxt时按顺序调用的**函数**。 框架在加载之前等待每个模块完成。 如此，模块几乎可以自定义Nuxt的任何地方。 感谢Nuxt的模块化设计 (基于 webpack [Tapable](https://github.com/webpack/tapable)), 模块可以轻松地为某些入口点注册钩子，例如构建器初始化。模块还可以覆盖模板，配置`webpack`加载器，添加`CSS`库以及执行许多其他有用的任务。
