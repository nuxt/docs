...
`nuxt.config.js`（または [CLI](https://nuxtjs.org/guide/commands#list-of-commands の ` -n`）に `server.socket` オプションを設定すると、 Unix ソケットも利用できます。
[Unix ソケット]（https://en.wikipedia.org/wiki/Berkeley_sockets）を使用する場合は、 `host` パラメータと ` port` パラメータを設定しないでください。設定した場合、 `socket` パラメータは無視されます。
