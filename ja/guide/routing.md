...
**ミドルウェアは `middleware/` ディレクトリに入れます。** ファイル名はミドルウェアの名前となります（`middleware/auth.js` は `auth` ミドルウェアになります）。ページ固有のミドルウェアを関数を直接使用しても定義することができます。[anonymous middleware](/api/pages-middleware#anonymous-middleware) を参照してください。
